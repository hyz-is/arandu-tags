...
| skill | when it fires |
| --- | --- |
| `tags-policy` | opening an action, adding an authorized Model-backed use case, or anything answering 403 |
| `tags-module` | adding a route, a handler, a config field, a response field or a migration |
| `tags-release` | the gates, the manifest, a dependency, a version, a tag |
| `tags-vault-notes` | writing the note, the gap or the journal entry, when this checkout sits inside the Arandu Obsidian vault |
| `tags-package` | installing and wiring this package **into an application** |
...
`tags-vault-notes` fires on a condition rather than on a task: it applies
