...
4. Only then does a `security.Grant` exist, and `service.go` reaches
   `Tags(s.db)` and a Model terminal with it.
...
`Tags(s.db)` call is therefore mandatory even though the terminal itself
also takes `g`.
...
`TagService` owns `db *data.DB`. Validate first, build the value the Policy
must see, call `security.Authorize`, and only then call `Tags(s.db)`.
...
```go
	...
	_, err = security.Authorize(ctx, s.policy, actor, TagView, *record)
```
