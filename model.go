...
func NewResource(record Skeleton) Resource {
	return newResource(record.ID, record.Name, record.CreatedAt)
}

func resourceFromPointer(record *Skeleton) Resource {
	if record == nil {
		return Resource{}
	}
	return newResource(record.ID, record.Name, record.CreatedAt)
...
func collectionFromPointers(records []*Skeleton, cursor string) Collection {
	resources := make([]Resource, 0, len(records))
	for _, record := range records {
		if record != nil {
